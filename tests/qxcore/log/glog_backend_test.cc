...
class GlogBackendTest : public ::testing::Test {
 protected:
  void SetUp() override {
    backend_ = std::make_unique<GlogBackend>();
  }
  ...
  std::unique_ptr<GlogBackend> backend_;
};

TEST_F(GlogBackendTest, Initialization) {
  // 测试初始化
  absl::Status status = backend_->init("test_glog", LogLevel::kDebug);
...
TEST_F(GlogBackendTest, DoubleInitialization) {
  // 测试重复初始化
  absl::Status status1 = backend_->init("test_glog", LogLevel::kInfo);
  EXPECT_TRUE(status1.ok());
  
  absl::Status status2 = backend_->init("test_glog2", LogLevel::kDebug);
  EXPECT_FALSE(status2.ok());
  EXPECT_EQ(status2.code(), absl::StatusCode::kAlreadyExists);
}

TEST_F(GlogBackendTest, SetLevel) {
  absl::Status status = backend_->init("test_glog", LogLevel::kInfo);
  EXPECT_TRUE(status.ok());
...
TEST_F(GlogBackendTest, SetLevelWithoutInitialization) {
  // 测试未初始化时设置级别
...
TEST_F(GlogBackendTest, BasicLogging) {
  absl::Status status = backend_->init("test_glog", LogLevel::kDebug);
  EXPECT_TRUE(status.ok());
...
TEST_F(GlogBackendTest, FormattedLogging) {
  absl::Status status = backend_->init("test_glog", LogLevel::kDebug);
  EXPECT_TRUE(status.ok());
...
TEST_F(GlogBackendTest, LogLevelFiltering) {
  absl::Status status = backend_->init("test_glog", LogLevel::kWarn);
  EXPECT_TRUE(status.ok());
...
TEST_F(GlogBackendTest, FlushAndShutdown) {
  absl::Status status = backend_->init("test_glog", LogLevel::kInfo);
  EXPECT_TRUE(status.ok());
...
TEST_F(GlogBackendTest, LoggingWithoutInitialization) {
  // 测试未初始化时的日志记录
